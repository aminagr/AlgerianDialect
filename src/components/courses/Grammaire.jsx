...
const Grammaire = () => {
  ...
  // Définir l'état initial dans useEffect
  useEffect(() => {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      setSearchTerm(searchTerm); // Vérifie si c'est vraiment nécessaire
    }
  };
  ...
  const renderCourseCards = () => {
    return Object.keys(lessonsData.courses)
      .filter(courseId => lessonsData.courses[courseId].type === "Grammaire") 
      .map((courseId, index) => (
        <CourseCard
          key={index}
